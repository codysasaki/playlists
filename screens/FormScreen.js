...
const FormScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    if (routeParamsExist) {
      ...
      navigation.setOptions({
        title: "Edit Playlist",
        headerRight: () => (
          <Button onPress={() => deletePlaylist()} color="#00cc00">
            Delete
          </Button>
        ),
      });
      ...
      for (let id in songs) {
        if (routeParamsExist) {
          if (routeParamsExist.data.songs) {
            const selectedSongObject = route.params.data.songs.filter(
              (item) => item.id == songs[id].id
            );
  ...
  function writeUserPlaylist(playlist) {
    const newPlaylistKey = push(
      child(ref(db), "users/" + userId + "/playlists")
    ).key;
